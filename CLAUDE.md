...
## 13) Python SDK 护栏
- 主模块：`python/openjax_sdk/src/openjax_sdk/client.py`。
- 保持协议兼容（请求/响应 envelope 字段与 `openjaxd` 对齐）。
- 保持异常分层稳定（`OpenJaxProtocolError` / `OpenJaxResponseError`）。
- 在改动事件流处理前，补齐集成测试。
