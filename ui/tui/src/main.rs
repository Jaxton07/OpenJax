...
use tui::app::{App, SubmitAction};
use tui::approval::TuiApprovalHandler;
use tui::input::{InputAction, map_event};
use tui::tui::Tui;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    if std::env::var("OPENJAX_TUI_RUNTIME")
        .ok()
        .is_some_and(|v| v.eq_ignore_ascii_case("next"))
    {
        return tui_next::run().await;
    }

    run_legacy().await
}

async fn run_legacy() -> anyhow::Result<()> {
    init_logger();
