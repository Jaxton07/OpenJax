...
| 任务 | 说明 | 预计工时 | 测试要求 |
|------|------|----------|----------|
| 5.1 ToolCard 摘要卡片 | 实现工具调用结果摘要卡片（摘要优先） | 3h | 单元测试：卡片渲染 |
| 5.2 Markdown 渲染 | 集成助手消息 Markdown 渲染 | 3h | 单元测试：Markdown 解析 |
| 5.3 代码块高亮 | 代码块语法高亮 | 2h | 人工测试：视觉效果 |
| 5.4 样式与可读性打磨 | ToolCard + Markdown 视觉收敛与可读性优化 | 1.5h | 人工测试：布局与可读性 |

**交付物**:
- ToolCard 组件显示工具摘要（不含折叠/展开）
- Markdown 消息自动渲染
- 代码块语法高亮
...
| 任务 | 说明 | 预计工时 | 测试要求 |
|------|------|----------|----------|
| 6.1 配置文件 | 支持 `~/.config/openjax/config.toml` | 2h | 单元测试：配置读取 |
| 6.2 测试覆盖 | 补充遗漏的单元测试 | 3h | 覆盖率 > 80% |
| 6.3 文档 | 更新 README 和使用文档 | 2h | - |
