...
- [x] 新建 `python/openjax_sdk`（2026-02-20）
- [x] 封装 daemon 生命周期管理（启动/健康检查/退出）（2026-02-20）
- [x] 提供 async 客户端（sync 可后补）（2026-02-20）
- [x] 实现事件分发与 `AssistantDelta` 聚合（2026-02-20）
- [x] 实现审批接口映射（2026-02-20）
- [x] 增加协议类型约束（Pydantic 或 dataclass）（2026-02-20，采用 dataclass）
- [x] 增加单元测试与端到端集成测试（2026-02-20）
...
- [x] 新建 `python/openjax_tui`（2026-02-20）
- [x] 实现输入、流式输出、审批弹层（2026-02-20，MVP 命令式审批）
- [x] 对齐关键键位和错误提示（2026-02-20，`y/n` 快捷审批 + 错误回显）
- [x] 做终端恢复兜底（异常退出场景）（2026-02-20，补充恢复说明与异常路径处理）
- [ ] 完成 tmux/zellij 基础回归
- [x] 保留 Rust TUI 作为可切换回退路径（2026-02-20，`openjax-tui` 继续保留）
...
- 2026-02-20：完成阶段 D（Python SDK MVP + async e2e 测试通过）。
- 2026-02-20：阶段 E 启动，`python/openjax_tui` MVP 可在终端运行并打通 submit/event/approval 命令链路。
- 2026-02-20：阶段 E 继续推进，补充快捷审批/错误提示并新增 `python/openjax_tui` smoke test。
- 2026-02-20：补充 Python TUI 终端恢复文档、tmux/zellij 回归清单和 smoke 脚本。
- 2026-02-20：新增复用器环境检查脚本与回归报告模板，阶段 E 剩余人工 tmux/zellij 实测。
