...
| `minimax_coding` | MiniMax Coding | `https://api.minimax.com/anthropic/v1` | `anthropic_messages` | `OPENJAX_MINIMAX_API_KEY` |
...
- MiniMax：`base_url` 改为 `https://api.minimax.com/anthropic/v1`（国内地址，`.com` 非 `.io`），`protocol` 改为 `anthropic_messages`
...
**4c. `max_tokens` 始终设置**：
使用固定默认值 `32000`（与 OpenCode `ProviderTransform.OUTPUT_TOKEN_MAX = 32000` 对齐）。
当前代码 `max_tokens: request.options.max_output_tokens.unwrap_or(4096)`，改为 `.unwrap_or(32000)`。
注：`RegisteredModel` 不携带 `context_window_size`，env 路径也无此信息，故不做 `min()` 计算，直接用 32000 作为上限。

**4d. temperature 改为 `None`**：
当前 `build_request` 中硬编码 `temperature: Some(0.2)`（[anthropic_messages.rs:265](openjax-core/src/model/anthropic_messages.rs)），需改为 `None`，由 provider 使用自身默认值。
同时 `AnthropicMessagesRequest.temperature` 字段已是 `Option<f32>`，不需要结构体改动，只需改 `build_request` 中的赋值。
...
**5c. `max_tokens` 始终设置**：
同 Task 4c，使用固定默认值 `32000`。当前 `ChatCompletionRequest.temperature` 是 `f32`（非 Optional），需同步改为 `Option<f32>` 并设默认 `None`。
...
### Task 7：清理 env/config 路径——迁移 MiniMax + 删除 GLM legacy

本次重构范围包含 env/config 路径（`factory.rs` 的 fallback 链），需与 DB 路径保持一致。

**7a. MiniMax env 路径迁移**

- `openjax-core/src/model/anthropic_messages.rs`：新增 `from_minimax_config()`，使用
  - `api_key_env = "OPENJAX_MINIMAX_API_KEY"`
  - `model_env = "OPENJAX_MINIMAX_MODEL"`
  - `base_url_env = "OPENJAX_MINIMAX_BASE_URL"`
  - 默认 model = `"MiniMax-M2.7"`（与 catalog 对齐）
  - 默认 base_url = `"https://api.minimax.com/anthropic/v1"`
- `openjax-core/src/model/chat_completions.rs`：删除 `from_minimax_config()`（MiniMax 不再走 chat_completions）
- `openjax-core/src/model/factory.rs`：将 `ChatCompletionsClient::from_minimax_config(config)` 替换为 `AnthropicMessagesClient::from_minimax_config(config)`

**7b. GLM legacy 路径清理**

- `openjax-core/src/model/anthropic_messages.rs`：删除 `is_legacy_glm_chat_base_url()` 函数及 `from_glm_config()` 中的 legacy URL 检测逻辑（直接用 default_base_url 或 config/env 值）
- `openjax-core/src/model/chat_completions.rs`：删除 `from_glm_config()`——该函数在 `factory.rs` 中已被 `AnthropicMessagesClient::from_glm_config`（line 60）抢先处理，line 66 的 chat_completions GLM 路径是死代码
- `openjax-core/src/model/factory.rs`：删除 `ChatCompletionsClient::from_glm_config(config)` 这一行

**7c. registry.rs 协议推断更新**

- `openjax-core/src/model/registry.rs`：`infer_protocol()` 函数中增加 `"kimi" | "minimax"` → `"anthropic_messages"` 映射（覆盖 legacy config 桥接路径）
- `default_model_name()` 中 `minimax` 默认 model 改为 `"MiniMax-M2.7"`
- `infer_request_profile()` 中 Kimi 的 `kimi_coding_v1` 推断逻辑可以删除（Kimi 迁移到 anthropic_messages 后会走 `anthropic_default`）
...
Task 7 (env 路径清理) — 依赖 Task 3 确认 GLM/MiniMax 走新协议
```

推荐执行顺序：**Task 3 → Task 4 → Task 5 → Task 7 → Task 1 → Task 2 → Task 6**

- Task 3/4/5/7 无 DB 变更，风险低，先做可以验证请求格式是否正确
- Task 7 依赖 Task 3 定下 MiniMax/GLM 的目标协议
- Task 1/2/6 涉及 DB migration 和 WebUI，最后做
...
| `minimax` | `https://api.minimax.io/anthropic/v1`（国际）/ `https://api.minimax.com/anthropic/v1`（国内）| `@ai-sdk/anthropic` → `anthropic_messages` |
...
openjax-core/
  ...
  src/model/anthropic_messages.rs — 增加 anthropic-beta header，统一 User-Agent，max_tokens 默认值，temperature→None，新增 from_minimax_config，清理 GLM legacy
  src/model/chat_completions.rs   — temperature→Option<f32>/None，统一 User-Agent，max_tokens 默认值，删除 from_minimax_config/from_glm_config
  src/model/request_profiles/     — kimi_coding_v1 profile 移除 User-Agent 覆盖
  src/model/factory.rs            — 更新 MiniMax env 路径，删除 GLM chat_completions 死代码行
  src/model/registry.rs           — infer_protocol/default_model_name/infer_request_profile 同步更新
